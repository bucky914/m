...
async function showEnrollPanel(planId) {
  ...
  document.getElementById('enrollForm').addEventListener('submit', async (e) => {
    ...
    // Enrollment is now automatic — no admin approval step. The
    // subscription goes straight to 'active' with washes_remaining
    // seeded from the plan and start_date set to today.
    const { error: insertError } = await supabaseClient.from('subscriptions').insert({
      ...
      washes_used: 0,
      bonus_perk_used: false,
      mid_year_reset_used: false,
    ...
    if (insertError) {
      alert('Something went wrong activating your membership: ' + insertError.message);
...
function showEnrollSuccessBanner() {
  ...
  banner.textContent = '✓ Your membership is active. You can book your next maintenance visit now.';
...
async function openVisitModal(subscriptionId) {
  ...
  // Clear any stale per-date cache and render the calendar. Individual
  // dates are checked for availability on demand as the customer
  // navigates the calendar (see loadDateAvailability()), not all at once.
  ...
  document.getElementById('visitForm').addEventListener('submit', async (e) => {
    ...
    if (error) {
      ...
      if (message.includes('SLOT_CONFLICT') || message.includes('DATE_FULLY_BOOKED')) {
        ...
        renderVisitCalendar();
        resetDateSelection();
        alert('This time slot was just booked by another customer. Please choose another available time.');
        return;
      }
